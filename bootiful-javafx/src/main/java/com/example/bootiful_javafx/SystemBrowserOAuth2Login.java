...
record UserSignedInEvent(OAuth2AuthenticationToken authentication) {
    ...
    String name() {
        return StringUtils.hasText(user().getPreferredUsername()) ?
                user().getPreferredUsername() : user().getName();
    }
}

/* Plugs the interactive sign-in into the OAuth2AuthorizedClientManager, as the last resort behind the refresh token grant */
@Component
